...
Production data engineering patterns: Delta Lake Medallion, Kafka streaming, dbt quality gates. 90 runnable tests.
...
Three production-ready modules backed by **90 pure Python tests** (no Spark runtime needed) — this is the runnable claim in this repo:

| Module | Pattern | Key Feature |
|--------|---------|-------------|
| [`patterns/medallion_pipeline.py`](patterns/medallion_pipeline.py) | Bronze/Silver/Gold | Z-ORDER benchmark harness + SLA-tracked Gold aggregation |
...
```bash
pip install pytest
pytest patterns/tests/ -v   # 90 tests, all pass, no Spark required
```

> **Reference benchmark (external Databricks run):** The 82% Z-ORDER speedup (14.3s → 2.6s, 1B rows) quoted throughout this README was recorded from a Databricks DBR 14.3 run on the environment listed in the `ZORDER_BENCHMARK_RESULT` constant in [`patterns/medallion_pipeline.py`](patterns/medallion_pipeline.py) — it is NOT re-derivable in this repo's CI (no Spark cluster available) and is retained as a reference data point, not a claim this repo proves. (Distinct from the 10M-row Z-ORDER row in the benchmark table below — same optimization class at a different scale; the matching 82% figure is coincidental, not recycled.)
...
_Reference results from a Databricks run (see [benchmarks/RESULTS.md](benchmarks/RESULTS.md) for methodology); requires pyspark + a Databricks cluster to re-run — not executed in CI. CI verifies the 90 pure-Python pattern tests._
...
Default — 90 pure-Python tests, no external dependencies, this is what CI runs:

```bash
pip install pytest
PYTHONPATH=. pytest patterns/tests/ -v
```

Expected output (abridged):
```
patterns/tests/test_medallion_pipeline.py::... PASSED
patterns/tests/test_streaming_kafka.py::... PASSED
patterns/tests/test_dbt_transformations.py::... PASSED
90 passed in 0.04s
```

Optional — legacy Spark-backed suite (`tests/test_patterns.py`). Requires `pip install pyspark delta-spark`; not run in CI:
...
These patterns generalize failure classes seen across production data platforms; the scenarios below are composite and fictionalized:
