...
class TestClassifyBenchmarkVerdict:
    ...
    def test_documented_result_is_beneficial(self) -> None:
        """Check INTERNAL CONSISTENCY of the recorded ZORDER_BENCHMARK_RESULT constant only.

        This does NOT run or re-derive the underlying Databricks benchmark -- that
        result is an external reference data point (see the constant's docstring
        in patterns/medallion_pipeline.py). This test only verifies that the
        recorded before/after numbers arithmetically classify as BENEFICIAL via
        this module's own compute_improvement_pct/classify_benchmark_verdict
        functions, i.e. the constant hasn't drifted into internal inconsistency.
        """
