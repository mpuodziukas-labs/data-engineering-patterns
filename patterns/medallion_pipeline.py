...
- Z-ORDER optimization with before/after benchmark harness
...
Reference benchmark (external Databricks run, NOT re-derivable in this repo's
CI -- no Spark cluster available here; recorded from a Databricks DBR 14.3 run
on the environment described in ZORDER_BENCHMARK_RESULT below, retained as a
reference data point only):
...
# Reference benchmark recorded from a Databricks DBR 14.3 run on the cluster
# and table described below. NOT re-derivable in this repo's CI (no Spark
# cluster available) -- retained as a reference data point, not a claim this
# repo's own tests prove. See patterns/tests/test_medallion_pipeline.py for
# the internal-consistency check that runs against this constant.
